...
void setupAsyncServer(AsyncWebServer &server) {
    ...
    server.on(
        ...
        [](AsyncWebServerRequest *request) {
            ...
            AsyncWebServerResponse *response = request->beginResponse(
                200, "text/html", rebooting ? ok_html : failed_html);
    ...
    server.on(relP("/compDate"), HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "text/plain", __DATE__ " " __TIME__);
    });

    server.on(
        relP("/upload"), HTTP_POST,
        [](AsyncWebServerRequest *request) { request->send(200); }, uploadFile);
    ...
    server.on(relP("/download"), HTTP_GET, [](AsyncWebServerRequest *request) {
        if (!authenticate(request)) return;
